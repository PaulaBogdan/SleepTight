...
typealias SignInWithAppleResult = (authDataResult: AuthDataResult,
...
struct FBAuth {
  ...
  static func handle(_ signInWithAppleResult: SignInWithAppleResult, completion: @escaping (Result<Bool, Error>) -> ()) {
    ...
    // Now it's possible to update User database to add this user.
    ...
    let data = FBUser.dataDict(uid: uid,
                               name: givenName,
                               email: email, favoritesSongs: favoritesSongs, recentSongs: recentSongs)
  ...
  static func reauthenticateWithGoogle(idToken: String, accessToken: String, completion: @escaping (Result<Bool,Error>) -> Void) {
